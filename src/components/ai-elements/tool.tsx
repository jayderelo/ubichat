...
export const getStatusBadge = (status: ToolPart["state"]) => (
  <Badge className="h-5 gap-1 rounded-full px-1.5 text-[11px]" variant="secondary">
    {statusIcons[status]}
    {statusLabels[status]}
  </Badge>
);
...
export const ToolHeader = ({
  ...
  return (
    <CollapsibleTrigger
      className={cn("flex w-full items-center justify-between gap-2 px-2.5 py-2", className)}
      {...props}
    >
      <div className="flex min-w-0 items-center gap-1.5">
        <WrenchIcon className="size-3.5 shrink-0 text-muted-foreground" />
        <span className="truncate font-medium text-xs">{title ?? derivedName}</span>
